...
interface SpringDataOutboxRepository extends JpaRepository<OutboxMessageJpaEntity, UUID> {
     ...
     * conclusões exigem depois. {@code gen_random_uuid()} é v4, e está certo assim: o token é efêmero e
     * nunca indexado, então a ordenação temporal do v7 não compraria nada.
     * <p>
     * Sem {@code @Modifying} de propósito: a query devolve linhas, e o Spring Data precisa mapeá-las. Isso
     * funciona porque o driver JDBC aceita {@code executeQuery} sobre {@code UPDATE ... RETURNING}.
     */
    @Query(
            value = """
                    WITH candidates AS (
