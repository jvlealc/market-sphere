...
public class Invoice {
    ...
    public boolean markAsGenerated(String storageKey, Instant generatedAt) {
        ...
        String normalizedStorageKey = requireNonBlank(storageKey, "storageKey");
        Instant requiredGeneratedAt = requireNonNull(generatedAt, "generatedAt");
    ...
    public boolean markAsFailed(String failureReason, Instant failedAt) {
        ...
        Instant requiredFailedAt = requireNonNull(failedAt, "failedAt");
    ...
    private static void validateNewInvoice(UUID id, Long orderId) {
        if (id == null) {
            throw new InvalidInvoiceException("id must not be null");
        }
    ...
    private static void validateOrderId(
        ...
        if (orderId == null) {
            throw exceptionFactory.apply("orderId must not be null");
        }

        if (orderId <= 0L) {
            throw exceptionFactory.apply("orderId must be greater than zero");
        }
    }
